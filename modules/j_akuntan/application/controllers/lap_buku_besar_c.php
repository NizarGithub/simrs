...
class Lap_buku_besar_c extends CI_Controller {
	...
	function index()
	{
		...
		if($this->input->post('pdf')){
			$this->cetak_buku_besar_pdf();
		} else if($this->input->post('excel')){
			$this->cetak_buku_besar_xls();
		} 

		//$dt = $this->model->get_no_akun($keyword, $id_klien);

		$data =  array(
			'page' => "lap_buku_besar_v", 
			...
			//'dt' => $dt, 
			'msg' => $msg, 
			'post_url' => 'lap_buku_besar_c', 
		);
		
		$this->load->view('beranda_v', $data);
	}

	function cetak_buku_besar_pdf(){
		$sess_user = $this->session->userdata('masuk_rs');
		$id_klien = $sess_user['id_klien'];
		...
		$view = "pdf/report_buku_besar_pdf";
		$dt = "";
		$judul = "";
		...
		if($filter == "Harian"){
			...
			$dt = $this->model->get_lap_buku_besar($id_klien, $tgl_awal, $tgl_akhir);
			...
			$dt = $this->model->get_lap_buku_besar_bulanan($id_klien, $bulan, $tahun);
		} 
	...
	function cetak_buku_besar_xls(){
		...
		$filter = $this->input->post('filter');
		$view = "xls/report_buku_besar_xls";
		$dt = "";
		$judul = "";
