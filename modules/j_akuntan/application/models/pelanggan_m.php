...
class Pelanggan_m extends CI_Model
{
    ...
    function hapus_pelanggan($id){
        $sql = "
        DELETE FROM ak_pelanggan WHERE ID = $id
        ";
    ...
    function simpan_pelanggan($id_klien, $nama_pelanggan, $npwp, $alamat_tagih, $alamat_kirim, $no_telp, $no_hp, $email, $tipe, $nama_usaha, $tdp, $siup){
        $tgl = date('d-m-Y');
        $jam = date('H:i');
        $waktu = $tgl.", ".$jam;
        ...
        (ID_KLIEN, NAMA_PELANGGAN, NPWP, ALAMAT_TAGIH, ALAMAT_KIRIM, NO_TELP, NO_HP, EMAIL, WAKTU, WAKTU_EDIT, TIPE, NAMA_USAHA, TDP, SIUP)
        VALUES 
        ($id_klien, '$nama_pelanggan', '$npwp', '$alamat_tagih', '$alamat_kirim', '$no_telp', '$no_hp', '$email', '$waktu', '-', '$tipe', '$nama_usaha', '$tdp', '$siup')
        ";
