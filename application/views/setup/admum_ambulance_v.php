...
$(document).ready(function(){
	...
	$('#btn_ya_prwt').click(function(){
		...
		$.ajax({
			url : '<?php echo base_url(); ?>setup/admum_ambulance_c/hapus_perawat',
...
function get_kode(){
	$.ajax({
		url : '<?php echo base_url(); ?>setup/admum_ambulance_c/get_kode',
		type : "POST",
		dataType : "json",
...
function klik_pegawai(id){
	...
	$.ajax({
		url : '<?php echo base_url(); ?>setup/admum_ambulance_c/klik_pegawai',
		data : {id:id},
		type : "POST",
		dataType : "json",
...
function paging($selector){
    ...
    {
        $selector = $("#tabel_ambulance tbody tr");
    }
...
function data_ambulance(){
	...
	$.ajax({
		url : '<?php echo base_url(); ?>setup/admum_ambulance_c/data_ambulance',
		data : {keyword:keyword},
		type : "POST",
		dataType : "json",
		success : function(result){
...
function ubah_ambulance(id){
	...
	$.ajax({
		url : '<?php echo base_url(); ?>setup/admum_ambulance_c/data_detail_perawat',
		data : {id:id},
		type : "POST",
		dataType : "json",
		success : function(result){
...
function hapus_ambulance(id){
	$('#popup_hapus').click();

	$.ajax({
		url : '<?php echo base_url(); ?>setup/admum_ambulance_c/data_ambulance_id',
		data : {id:id},
		type : "POST",
		dataType : "json",
		success : function(row){
			$('#id_hapus').val(id);
			var text = row['KODE']+' - '+row['NOMOR_PLAT'];
            $('#msg').html('Apakah Ambulance ini <b>'+text+'</b> ingin dihapus?');
		}
	});
}
...
function hapus_perawat(id_perawat,id){
	...
	$.ajax({
		url : '<?php echo base_url(); ?>setup/admum_ambulance_c/data_perawat_id',
...
function detail_perawat(id){
	...
	$.ajax({
		...
		dataType : "json",
		success : function(result){
			$('#kode_detail').val(result['ambulance']['KODE']);
			$('#nomor_plat_detail').val(result['ambulance']['NOMOR_PLAT']);
			...
			if(result['perawat'] == "" || result['perawat'] == null){
				...
				for(var i=0; i<result['perawat'].length; i++){
					...
					$tr += "<tr>"+
								"<td style='text-align:center;'>"+no+"</td>"+
								"<td>"+result['perawat'][i].NAMA_PERAWAT+"</td>"+
								"<td>"+result['perawat'][i].NAMA_DEP2+"</td>"+
								"<td>"+result['perawat'][i].NAMA_DIV2+"</td>"+
								"<td>"+result['perawat'][i].JABATAN+"</td>"+
							"</tr>";
				}
			}
		...
		<div class="row">
			<ul class="nav nav-tabs">
                <li role="presentation" class="active">
                    <a href="#home1" role="tab" data-toggle="tab"><i class="fa fa-ambulance"></i> Data Ambulance</a>
                </li>
            ...
            <div class="tab-content">
                <div role="tabpanel" class="tab-pane fade in active" id="home1">
	                	...
	                	<div class="form-group">
			                ...
			                <div class="col-md-4">
				                ...
				                <div class="input-group">
				                    <input type="text" class="form-control" id="cari_prwt" placeholder="Cari..." value="" onkeypress="return onEnterText(event);">
                    ...
                    <div class="table-responsive">
			            <table id="tabel_ambulance" class="table table-bordered">
			                <thead>
			                    <tr class="biru">
			                        <th style="color:#fff; text-align:center;">No</th>
			                        <th style="color:#fff; text-align:center;">Kode</th>
			                        <th style="color:#fff; text-align:center;">Nomor Plat</th>
			                        <th style="color:#fff; text-align:center;">Sopir</th>
				        ...
				        <div class="card-box">
				            <div class="row">
			                    ...
			                    <div class="form-group">
			                        ...
			                        <div class="col-md-5">
			                            <input type="text" class="form-control" name="nomor_plat" id="nomor_plat" value="" required="required">
			                        </div>
			                    </div>
			                    <div class="form-group">
			                        ...
			                        <div class="col-md-5">
			                            <div class="input-group">
			                            	<input type="hidden" name="id_sopir" id="id_sopir" value="">
			                                <input type="text" class="form-control" name="sopir" id="sopir" value="" readonly="readonly" required="required">
			                                <span class="input-group-btn">
			                    ...
			                    <div class="form-group">
			                        ...
			                        <div class="col-md-5">
			                            <div class="input-group">
			                                ...
			                                <span class="input-group-btn">
			                                    <button type="button" class="btn btn-warning btn_perawat"><i class="fa fa-search"></i></button>
			                                </span>
			                            </div>
			                        </div>
			                    </div>
...
<button class="btn btn-primary" data-toggle="modal" data-target="#myModal4" id="popup_detail_perawat" style="display:none;">Standard Modal</button>
<div id="myModal4" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="custom-width-modalLabel" aria-hidden="true" style="display: none;">
    <div class="modal-dialog" style="width:55%;">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                <h4 class="modal-title" id="myModalLabel">Ambulance & Perawat</h4>
            </div>
            <div class="modal-body">
                <form class="form-horizontal" role="form">
		            <div class="form-group">
		            	<label class="col-md-2 control-label">Ambulance</label>
		                <div class="col-md-6">
		                    <input type="text" class="form-control" id="kode_detail" value="" readonly="readonly">
		                </div>
		            </div>
		            <div class="form-group">
		            	<label class="col-md-2 control-label">Nomor Plat</label>
		                <div class="col-md-6">
		                    <input type="text" class="form-control" id="nomor_plat_detail" value="" readonly="readonly">
		                </div>
		            </div>
		        </form>
            	<div class="table-responsive">
            		<div class="scroll-y">
		                <table class="table table-hover table-bordered" id="tabel_detail_perawat">
		                    <thead>
		                        <tr class="biru_popup">
		                            <th style="text-align:center; color: #fff;" width="50">No</th>
		                            <th style="text-align:center; color: #fff;">Nama Perawat</th>
