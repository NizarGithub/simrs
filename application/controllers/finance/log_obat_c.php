...
class Log_obat_c extends CI_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->model('logistik/log_obat_m','model');
	...
	{
		$data = array(
			'page' => 'logistik/log_obat_v',
			'title' => 'Gudang Obat',
			'subtitle' => 'Gudang Obat',
			'master_menu' => 'obat',
			'view' => 'obat',
			'url_simpan' => base_url().'logistik/log_obat_c/simpan',
			'url_ubah' => base_url().'logistik/log_obat_c/ubah',
			'url_hapus' => base_url().'logistik/log_obat_c/hapus',
		);
	...
	function simpan(){
		...
		$sql_cek = "SELECT COUNT(*) AS TOTAL FROM log_gudang_obat WHERE ID_SETUP_NAMA_OBAT = '$id_nama_obat'";
		...
		if($total_data == 0){
			...
			$aktif = '0';
			$sql = "SELECT * FROM log_gudang_obat WHERE ID_SETUP_NAMA_OBAT = '$id_nama_obat' ORDER BY URUT_BARANG DESC LIMIT 1";
	...
	function ubah(){
		...
		$kadaluarsa = str_replace(',', '', $this->input->post('tanggal_expired_ubah'));
		$gambar = "";

		if(!empty($_FILES['fileuser']['name'])){
			$gambar = $_FILES['fileuser']['name'];
			@unlink("./files/foto_obat/".$this->input->post('file_hidden'));
			$this->upload('fileuser');
		}else{
			$gambar = $this->input->post('file_hidden');
		}

		$this->model->ubah($id,$id_nama_obat,$id_jenis,$id_satuan,$jumlah,$isi,$total,$jumlah_butir,$harga_beli,$harga_jual,$kadaluarsa,$gambar);
